...
TASK_KEYWORDS = ['Images', 'object detection',
                 'label verification', 'bounding boxes']
...
PUBLIC_WORKTEAM_ARN = f"arn:aws:sagemaker:{region}:394669845002:workteam/public-crowd/default"

LabelingjobConfig = namedtuple("LabelingjobConfig", "input_manifest use_private_workteam execution_id role workteam_arn instructions_template_s3_uri label_categories_uri output_uri")
...
def  get_step_config(event)-> LabelingjobConfig: 
    ...
    instructions_template_s3_uri = f's3://{bucket}/{bucket_prefix}/groundtruth/verification_job/template.liquid'
    label_categories_uri = f's3://{bucket}/{bucket_prefix}/groundtruth/verification_job/data.json'
    output_uri = f's3://{bucket}/{bucket_prefix}/runs/{execution_id}/'

    return LabelingjobConfig(input_manifest, use_private_workteam, execution_id, role, workteam_arn, instructions_template_s3_uri, label_categories_uri, output_uri)
...
def create_labeling_job_config(config: LabelingjobConfig):
    ...
    ground_truth_request = {
        "InputConfig": {
            "DataSource": {
                "S3DataSource": {
                    "ManifestS3Uri": config.input_manifest
                }
            },
            "DataAttributes": {
                "ContentClassifiers": [
