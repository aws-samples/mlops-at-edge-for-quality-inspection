...
LambdaConfig = namedtuple('LambdaConfig', [ 'role' , 'verification_job_output','feature_group_name','feature_name_s3uri','feature_store_s3uri',  'query_results_s3uri' ])
def initialize_lambda_config(event):
    ...
    return LambdaConfig(role, verification_job_output, feature_group_name,feature_name_s3uri, feature_store_s3uri, query_results_s3uri)
...
def handler(event, context):
    ...
    for record in records:
        ...
        response = sagemaker_featurestore_runtime_client.put_record(FeatureGroupName=config.feature_group_name, 
                                                                Record=record)
    logging.info("Updated feature store with new labels")

def transform_df_to_records(df : pd.DataFrame):
    return [[ {'ValueAsString': str(row[column]), 'FeatureName': column} for column in df.columns ]  for _, row in df.iterrows()]
...
def load_manifest_to_dataframe(s3_uri):
    ...
    for line in file_content.splitlines():
        ...
        time = datetime.datetime.strptime(
            item['category-metadata']['creation-date'], "%Y-%m-%dT%H:%M:%S.%f").timestamp()
        # strftime("%Y-%m-%dT%H:%M:%S%f"))
        creation_date.append(int(round(time)))
        labeling_job.append(item['category-metadata']['job-name'])

        item_status = "APPROVED" if item['verified'] == VERIFICATION_APPROVED_CLASS else "REJECTED"
        status.append(item_status)

    data_frame = pd.DataFrame({"source_ref": source_ref,
    ...
    for label in data_frame.columns:
        if data_frame.dtypes[label] == object:
            ...
            data_frame[label] = data_frame[label].astype(
                "str").astype("string")
