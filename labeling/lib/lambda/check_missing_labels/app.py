...
LambdaConfig = namedtuple('LambdaConfig', ['feature_group_name',  'feature_name_s3uri', 'input_images_s3uri', 'query_results_s3uri', ])
def initialize_lambda_config():
    feature_group_name = os.environ["FEATURE_GROUP_NAME"] if "FEATURE_GROUP_NAME" in os.environ else "tag-quality-inspection"
    feature_name_s3uri = os.environ["FEATURE_NAME_S3URI"] if "FEATURE_NAME_S3URI" in os.environ else 'source_ref'   
    input_images_s3uri = os.environ["INPUT_IMAGES_S3URI"] if "INPUT_IMAGES_S3URI" in os.environ else "s3://mymlopsbucket/pipeline/labeling/images/"
    query_results_s3uri = os.environ["QUERY_RESULTS_S3URI"] if "QUERY_RESULTS_S3URI" in os.environ else "s3://mymlopsbucket/tmp/feature_store_query_results"
    return LambdaConfig(feature_group_name,feature_name_s3uri,input_images_s3uri,query_results_s3uri)

lambda_config = initialize_lambda_config()

def handler(event, context):

    logger.info(f"check-missing-labels called with event {event} and lambda config {lambda_config}")

    bucket, key = split_s3_url(lambda_config.input_images_s3uri)
    images = get_list_of_files(
        bucket=bucket, prefix=key, file_types=['.jpg', '.png'])
    ...
    logger.info(f"Finished check-missing-labels lambda with {len(missing_labels)} missing labels")
    output = { "missing_labels_count" : len(missing_labels), "missing_labels":missing_labels}
    return  output
...
def get_images_without_labels(images=None, existing_labels=None):
    missing_labels = []
    for image in images:
        if not image in existing_labels:
