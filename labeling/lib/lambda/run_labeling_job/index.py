...
LabelingjobConfig = namedtuple("LabelingjobConfig", "missing_labels max_labels_per_labeling_job use_private_workteam execution_id role workteam_arn instructions_template_s3_uri label_categories_uri output_uri")
...
def handler(event, context):
    ...
    manifest_uri = create_and_upload_manifest(
        images=missing_labels,  manifest_name=f'{config.execution_id}.manifest',  labeling_job_s3_uri=config.output_uri)
    # create labeling job
    ...
    return {"LabelingJobName": f'quality-inspection-labeling-job-{config.execution_id[:6]}'}
...
def  get_step_config(event)-> LabelingjobConfig: 
    ...
    instructions_template_s3_uri = f's3://{bucket}/{bucket_prefix}/groundtruth/labeling_job/instructions.template'
    label_categories_uri = f's3://{bucket}/{bucket_prefix}/groundtruth/labeling_job/class_labels.json'
    output_uri = f's3://{bucket}/{bucket_prefix}/runs/{execution_id}/'

    return LabelingjobConfig(missing_labels, max_labels_per_labeling_job, use_private_workteam, execution_id, role, workteam_arn, instructions_template_s3_uri, label_categories_uri, output_uri)

  


def create_and_upload_manifest(images=None, manifest_name="input.manifest", labeling_job_s3_uri=""):
    ...
    logger.info(
        f'Uploading manifest-file to s3://{bucket}/{prefix}/{manifest_name}')
    s3.meta.client.upload_file(Filename=tmp_file, Bucket=bucket,
