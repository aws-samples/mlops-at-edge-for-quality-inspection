...
def create(event, context):
    ...
    logger.info(
        f"check-missing-labels called with event {event}")
    ...
    for record in records:
        ...
        response = sagemaker_featurestore_runtime_client.put_record(FeatureGroupName=lambda_config.feature_group_name,
                                                                    Record=record)
...
def initialize_lambda_config(event: dict):
    feature_group_name = event['ResourceProperties']["feature_group_name"] if "feature_group_name" in event['ResourceProperties'] else "tag-quality-inspection"
    labels_uri = ""
