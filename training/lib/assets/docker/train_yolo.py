...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default="300,450",
        help="image data shape, specify using h,w",
    )
    ...
    parser.add_argument("--save-prefix", type=str,
                        default="", help="Saving parameter prefix")
    parser.add_argument(
        "--save-interval",
        type=int,
        default=10,
    ...
    args.data_shape = [int(x) for x in args.data_shape.split(",")]
...
def get_dataloader(net, train_dataset, val_dataset, data_shape, batch_size, num_workers, args):
    ...
    if args.no_random_shape:
        print(f"no random shape width {width} height {height}")
        print(f"Length training data set: len(train_dataset)")
        img, label = train_dataset[0]
        print(img.shape, label.shape)
        train_loader = gluon.data.DataLoader(
            train_dataset.transform(
                YOLO3DefaultTrainTransform(
                    width, height, net, mixup=args.mixup)
            ),
            batch_size,
            True,
...
def export_best_model(net, best_map, data_shape, prefix):

    net.load_parameters(prefix + "_best.params")
    print(f"Exporting final model to {os.environ['SM_MODEL_DIR']}/model")
    print(f"mAP={best_map[0]}")
    # save model
    net.set_nms(nms_thresh=0.35, nms_topk=400, post_nms=100)
    net(mx.nd.ones((1, 3, data_shape[0], data_shape[1]), ctx=ctx[0]))
...
def train(net, train_data, val_data, eval_metric, ctx, args):
        ...
        for i, batch in enumerate(train_data):
            ...
            fixed_targets = [
                gluon.utils.split_and_load(
                    batch[it], ctx_list=ctx, batch_axis=0)
                for it in range(1, 6)
            ]
            gt_boxes = gluon.utils.split_and_load(
                batch[6], ctx_list=ctx, batch_axis=0)
            sum_losses = []
            obj_losses = []
            center_losses = []
            scale_losses = []
    ...
    export_best_model(net, best_map, args.data_shape, args.save_prefix)
